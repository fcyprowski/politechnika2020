...
# 1
facts_sourcepath = DBI::dbGetQuery(con, "SELECT * FROM facts_sourcepath")
...
best_channels = DBI::dbGetQuery(
  con,
  "SELECT * FROM facts_result"
) %>%
  mutate_if(is.factor, as.character) %>%
  arrange(desc(total_conversions)) %>%
  slice(1:5) %>%
...
transition_matrix = DBI::dbGetQuery(
  con,
  "SELECT * FROM facts_transition_matrix"
) %>%
  mutate_if(is.factor, as.character) %>%
